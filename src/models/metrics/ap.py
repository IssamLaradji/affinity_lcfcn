...
class APMonitor:
    ...
    def get_avg_score(self, iouType='segm'):
        ...
        results["val_score"] = results['mAP50']
...
def compute_precision(gt_annList, pred_annList, iouType, iouThr, iouThrList):
    ...
    if len(gt_annList) == 0 or len(pred_annList) == 0:
        return {'mAP%d'%(iou*100):0 for iou in iouThrList}
...
def evaluate_annList(pred_annList, gt_annList,
    ...
    if ap == 1:
        result_dict["mAP"] = result_dict["mAP%d" % (iouThr * 100)]
    else:
        result_dict["mRC"] = result_dict["mRC%d" % (iouThr * 100)]
    return result_dict
...
def compute_AP_dict(ap, aind, mind, eval_dict, iouThr, iouThrList):
    ...
    if ap == 1:
        result_dict = {"AP%d" % (iouThr*100):np.array(per_class),
            "mAP%d"%(iouThr*100):mean_s}
    else:
        result_dict = {"RC%d" % (iouThr*100):np.array(per_class),
            "mRC%d"%(iouThr*100):mean_s}
