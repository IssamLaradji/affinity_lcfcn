...
cls_labels_dict = np.load(os.path.join(os.path.dirname(__file__),'cls_labels.npy'), allow_pickle=True).item()
...
class VOC12ClassificationDataset(VOC12ImageDataset):
    ...
    def __getitem__(self, idx):
        ...
        msf_dict = self.apply_msf(idx)
        out['img'] =  torch.from_numpy(out['img'])
        out['img_msf'] =  msf_dict['img']
        out['size'] = msf_dict['size']
        return out

    def apply_msf(self, idx):
        name = self.img_name_list[idx]
        name_str = decode_int_filename(name)
        ...
        out = {"name": name_str, 
                "img": torch.from_numpy(ms_img_list), 
...
class VOC12SegmentationDataset(Dataset):
    ...
    def __init__(self, img_name_list_path, label_dir, crop_size, voc12_root,
        ...
        self.img_name_list = load_img_name_list('/home/issam/Research_Ground/isps/scripts/irn/voc12/train.txt')
        self.voc12_root = voc12_root
