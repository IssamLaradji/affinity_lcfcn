...
class SumFish(data.Dataset):
    def __init__(self, split, datadir, exp_dict, habitat=None):
        ...
        if split == 'train':
            ...
            self.img_names, self.labels, self.mask_names = get_sum_data(datadir, "train_val", split)
            if self.count_mode:
            ...
            self.img_names, self.labels, self.mask_names = get_sum_data(datadir, "TEST", split)
            # self.img_names_other, self.labels_other= get_clf_data(datadir, split, habitat=habitat)
    ...
    def __getitem__(self, index):
        ...
        if self.split in ['train', 'val']:
            ...
            gt = rgb_loader(os.path.join(self.datadir, "train_val/masks", self.images[index].replace('.jpg', '.bmp')))
        elif  self.split == 'test':
            image = rgb_loader(os.path.join(self.datadir, "TEST/images", self.images[index]))
            gt = rgb_loader(os.path.join(self.datadir, "TEST/masks", self.images[index].replace('.jpg', '.bmp')))

        original = copy.deepcopy(image)

        image = self.img_transform(image)
        gt_org = self.gt_transform(gt)
        # gt = self.img_transform(gt)
        
        gt_name = self.gts[index]
        # hu.save_image('tmp.png', )
        gt = np.array(gt_org) / 255.
        gt_dec = bin2dec(gt)
        # hu.save_image('tmp.png', image, denorm='rgb', mask=(gt.sum(axis=2) == 1).astype('uint8'))
        # hu.save_image('tmp.png', image, denorm='rgb', mask=(gt_dec == 1).astype('uint8'))
        gt_mask = (gt_dec==6).astype('uint8')
        # gt = gt[:,:,0]
        # gt[gt==255] = 1

        # image,gt =  processSUIMDataRFHW(image, gt, sal=False)
        # _,gt =  processSUIMDfish(image, gt)
        ...
        points = lcfcn_loss.get_points_from_mask(gt_mask, bg_points=-1)
        ...
        batch = {'images': image,
                #  'image_other': image_other,
                 'original':original,
                 'masks': torch.as_tensor(gt_mask).long(),
        ...
        # pt_mask = expand_labels(points!=255, 10).astype('uint8')
        # pt_mask = label2rgb(pt_mask, bg_label=0)
        # hu.save_image('tmp.png', image, denorm='rgb', mask=pt_mask)
        return batch
...
# for sum,
def get_sum_data(path_base, splits, split):
    r = 0.8
    ...
    img_names = sorted(np.array(img))
    mask_names = sorted(np.array(msk))
    labels = np.array([1 for x in img])
    n = int(r * len(img_names))
    if split == 'train':
        return img_names[:n], labels[:n], mask_names[:n]
    if split == 'val':
        return img_names[n:], labels[n:], mask_names[n:]
